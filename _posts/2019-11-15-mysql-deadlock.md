...
查了一个线上mysql死锁的问题，记录一下以后别犯这么低级的错误
...
## 真相
从上面的建表语句可以看出auto_id是主键，业务id没有唯一索引。之前跑了这么久没有也确实没问题，因为之前客户端不会对同一个id重复请求。而上个月业务罗就修改，同一个id会有重复请求。所以执行`UPDATE task SET state=1 WHERE id='2234234'`时如果同一个id需要delete，那么必然会锁抢占，进入死锁了。
